...
def process(data_type, study, subject, read_dir, date_from,
    ...
    read_dir = os.path.join(read_dir, 'eyeTracking')
    ...
    for root_dir, dirs, files in os.walk(read_dir):
        ...
        for file_name in sorted(files):
            task, session_date, session_id, category = parse(file_name, subject, input_tz, output_tz)
            if task is None:
                continue

            data = get_data(task, session_date, session_id, category)
...
def count_files(df):
    df = df.groupby(['$date_to', 'session_id', 'category', 'task']).agg('sum').reset_index()
    return df.pivot_table(index=['$date_to', 'session_id', 'category'],
                         columns='task', values='counts').fillna(0).reset_index()

def get_data(task, session_date, session_id, category):
    df = {}
...
def parse(file_name, subject, input_tz, output_tz):
    ...
    if len(parsed_file_name) == 3 and file_name.endswith('mov'):
        task = 'movEYE'
        session_date = process_datetime(parsed_file_name[1], input_tz, output_tz)
        session_id = parsed_file_name[2][:-4]
        category = 'mov'
    elif len(parsed_file_name) == 6:
        task = 'edf' + parsed_file_name[4]
        session_date = process_datetime(parsed_file_name[1], input_tz, output_tz)
    ...
    return task, session_date, session_id, category
