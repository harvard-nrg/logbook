...
def process_seconds(df):
    ...
    df = df.pivot_table(index=['day','weekday',
        'timeofday','UTC_offset'], columns='event', values='timestamp').fillna(0)

    if 'cellular' in df.columns.name:
        df['cellular'] = df['cellular'].astype(int)
    if 'unreachable' in df.columns.name:
        df['unreachable'] = df['unreachable'].astype(int)
    if 'wifi' in df.columns.name:
