...
def process_seconds(df):
    ...
    df = df.groupby(['day','weekday', 'timeofday', 'UTC_offset', 'event']).count()
    df = df.pivot_table(index=['day','weekday', 'timeofday', 'UTC_offset'],
            columns='event', values='timestamp').fillna(0)

    try:
        df = df.rename(columns={
        "Device Idle (Doze) state change signal received; device in idle state." : "idle",
        "Device Idle (Doze) state change signal received; device not in idle state." : "not_idle",
        "Device shut down signal received" : "shutdown",
        "Power connected" : "power_connected",
        "Power disconnected" : "power_disconnected",
        "Screen turned off" : "screen_off",
        "Screen turned on" : "screen_on"
        })
        df[['idle', 'not_idle', 'shutdown', 'power_connected',
            'power_disconnected', 'screen_off', 'screen_on']] = df[['idle',
            'not_idle', 'shutdown', 'power_connected', 'power_disconnected',
            'screen_off', 'screen_on']].astype(int)
    ...
    try:
        ...
        df[['charging', 'full', 'locked',
            'unlocked', 'unplugged']] = df[['charging', 'full', 'locked',
            'unlocked', 'unplugged']].astype(int)
    except Exception as e:
        logger.debug(e)
        logger.debug('iOS')
